// @desc: Same triangles as triangle_triangle but stored as Convex with bbox storage disabled
#include <random>
#include <vector>
#include <iostream>
#include "pgl.hpp"
...
std::vector<pgl::Convex<Point>> randomConvexs(size_t n, int den) {
    std::mt19937 rgen(1);
...
void allPairs(const std::vector<pgl::Convex<Point>> &tris, Function f) {
    int n = 0;

    for(const auto& s : tris) {
        for(const auto& t : tris) {
            pgl::Convex<Point> s2 = s, t2 = t;
            auto b = f(s2,t2);
...
void run(Function f, int den=1) {
    auto tris = randomConvexs<Point>(2000,den);
    plf::nanotimer timer;
    timer.start();
    allPairs(tris, f);
