...
- [`powerDiagram(V)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#acdaa70fea88f1688d348bf191da38f13 "Computes the power diagram of a set of disks.") is the same diagram for a container of disks, measuring a site by the power distance $|x - c|^2 - r^2$ instead of the squared distance. The cells are still convex polygons, but a disk that its neighbors swallow owns no cell at all and a disk's center may fall outside its own cell, neither of which a point can do; disks of equal radius give the Voronoi diagram of their centers. Neither route the points have is open to it, so the edges come from cutting each of the $O(n^2)$ bisectors against every site, at $O(n^3 \log n)$.

- [`farthestVoronoiDiagram(V)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#acd245756f72c23fd64d5354a65c7d39b "Computes the farthest-point Voronoi diagram of a set of points.") returns the farthest-point Voronoi diagram of the points in `V`, every face labeled with the one site *farthest* from it. Only a vertex of the convex hull owns a cell, a site the hull contains being strictly farthest nowhere, and every cell is unbounded, so the diagram is a tree of segments and rays with $O(h)$ of them for $h$ hull vertices. Complexity $O(n \log n + h^3)$: the hull, then each of the $O(h^2)$ bisectors of what it leaves cut against all of it.

- [`voronoiDiagram(V, k)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a6cc9b13abeb83be524eebae5c690fe2a "Computes the Voronoi diagram of a set of points.") and [`powerDiagram(V, k)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#acdaa70fea88f1688d348bf191da38f13 "Computes the power diagram of a set of disks.") return the order-$k$ diagram instead: each face is labeled with the $k$ sites nearest to it, ordered by their position in `V`. The order must be between 1 and the number of sites. Cells that are empty never appear, so there are far fewer faces than $k$-element subsets. Over points the orders are built one on top of the next by Lee's refinement — each cell of the order-$(k-1)$ diagram is subdivided by the ordinary diagram of the sites it does not own; repeated or collinear sites have no refinement to take and fall back to the bisector route. Over disks it is always that route.
...
All methods use *triangular expansion*: the domain is triangulated once, then each vertex or query point runs a single traversal of the mesh carrying a cone of still unobstructed directions that every crossed diagonal clips, at a cost proportional to the visited part of the domain.
