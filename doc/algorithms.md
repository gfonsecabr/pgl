...
- [`powerDiagram(V)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a9e19a763e3b0668686d85e6503beecb2 "Computes the power diagram, or the order-k power diagram, of a set of disks.") does the same for a container of disks, measuring a site by the power distance $|x - c|^2 - r^2$ instead of the squared distance. The cells are still convex polygons, but a disk that its neighbors swallow owns no cell at all and a disk's center may fall outside its own cell, neither of which a point can do; disks of equal radius give the Voronoi diagram of their centers. There is no Delaunay route to take: the edges come from cutting each of the $O(n^2)$ bisectors against every site, at $O(n^3)$, and they are then overlaid the general way.

- [`voronoiDiagram(V, k)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a1f7e6ea679ab5e1047c6b90181116e97 "Computes the Voronoi diagram, or the order-k diagram, of a set of points.") and [`powerDiagram(V, k)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a9e19a763e3b0668686d85e6503beecb2 "Computes the power diagram, or the order-k power diagram, of a set of disks.") return the order-$k$ diagram instead: each face is labeled with the $k$ sites nearest to it, ordered by their position in `V`. The order must be between 1 and the number of sites, and $k = 1$ is the diagram above, so the label is a vector of sites in both cases. Cells that are empty never appear, so there are far fewer faces than $k$-element subsets. Complexity $O(n^3 \log n)$, plus an overlay quadratic in the number of edges: neither variant has a dual to borrow, so both take the bisector route above.
