...
- [`powerDiagram(V)`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a9e19a763e3b0668686d85e6503beecb2 "Computes the power diagram, or the order-k power diagram, of a set of disks.") does the same for a container of disks, measuring a site by the power distance $|x - c|^2 - r^2$ instead of the squared distance. The cells are still convex polygons, but a disk that its neighbors swallow owns no cell at all and a disk's center may fall outside its own cell, neither of which a point can do; disks of equal radius give the Voronoi diagram of their centers. There is no Delaunay route to take, so finding the edges costs $O(n^3)$.
