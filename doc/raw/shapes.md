...
- `l.integralLine<ResultNumber>()`: Only for rational coordinates. Returns an `std::optional` with an equal oriented line — same points, same direction — defined by two points with integer `ResultNumber` coordinates, so the predicates it feeds run without fractions. `ResultNumber` defaults to the integer type the coordinates' `Rational` is built on. The representative is canonical: its source is the lattice point of the line closest to the origin, with ties going to the lexicographically smaller one, and its target the next lattice point along the orientation. That source depends only on the point set, so every oriented line on it — either direction, whatever points define it — reports the same one. The optional is empty in three cases: the line misses the integer grid entirely, which is the common case rather than the exception — written as $ax + by = c$ with coprime integers $a$ and $b$, a line has integer points only when $c$ is an integer too, so $x = 1/2$ and $x + y = 1/2$ have none and no rescaling gives them any; a defining coordinate does not fit in `ResultNumber`, which asking for a wider integer type (up to `pgl::BigInt`) may fix; or the line is degenerate, hence undefined. The internal arithmetic is exact and overflow-free whatever the coordinate type, so an empty result always means one of those three and never a wrapped intermediate. Clearing denominators raises the coordinates to a high power, well past the single promotion step a predicate needs, so the width is chosen rather than assumed: the computation runs in a `pgl::int128` while every numerator and denominator is narrow enough to prove it fits, and in `pgl::BigInt` beyond that. Both paths run the same code and return the same line.
