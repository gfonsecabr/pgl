...
- `powerDiagram(V)` does the same for a container of disks, measuring a site by the power distance $|x - c|^2 - r^2$ instead of the squared distance. The cells are still convex polygons, but a disk that its neighbors swallow owns no cell at all and a disk's center may fall outside its own cell, neither of which a point can do; disks of equal radius give the Voronoi diagram of their centers. There is no Delaunay route to take: the edges come from cutting each of the $O(n^2)$ bisectors against every site, at $O(n^3)$, and they are then overlaid the general way.
