...
- Cells: `vertexCount()`, `halfedgeCount()`, `edgeCount()` (half the halfedge count) and `faceCount()` (the unbounded face included), plus the ranges `halfedges()` and `faces()` of every handle in index order. `vertices()` and `edges()` are the odd ones out: as for a shape, they return the geometry itself rather than handles — `vertices()` a `const std::vector<PointType>&` of the vertex positions in handle index order, `edges()` a `std::vector<Segment>` with one entry per twin pair, each carrying its edge label (built on demand, the topology keeping halfedges rather than edges). Build the vertex handles by counting up to `vertexCount()` when the incidence queries are what is wanted. Indexing with a handle gives the cell's geometry: `a[v]` is the position of a vertex, and `a[h]` is the edge of a halfedge as a `Segment` carrying the edge label — the same entry as `a.edges()[h.index() / 2]`. Twin halfedges give the same segment, since a segment is unoriented.

- Incidence: `twin(h)` is the halfedge running along the same edge the other way, `next(h)` the next one along the boundary of the face on the left, `origin(h)` and `target(h)` the vertices it leaves and reaches, and `face(h)` the face on its left. `outgoing(v)` returns one halfedge leaving a vertex, or the invalid handle when the vertex is isolated; it is the entry point of the rotational fan, since `next(twin(h))` is the next halfedge leaving the vertex clockwise.
...
- Faces: `isUnbounded(f)` tells the unbounded face apart, `outerCycle(f)` returns a halfedge of the face's counterclockwise outer cycle (the invalid handle for the unbounded face), and `innerCycles(f)` one halfedge of each clockwise inner cycle — the holes of the face, which for the unbounded face are the connected components of the input. `hasSimpleBoundary(f)` says whether the face has neither a hole nor an edge with the face on both sides.

- `polygon(f)` returns the closure of a bounded face as a `PolygonWithHoles`. The result is *regularized*: a dangling edge sticking into the face is dropped, and a boundary cycle that pinches shut at a vertex is cut there into one ring per side. The remaining vertices are kept as they are, so a vertex in the middle of a straight stretch of boundary stays in the ring.
