...
- `convexPartition()` cuts the domain into `Convex` pieces with pairwise disjoint interiors whose union is the domain, by Hertel–Mehlhorn: delete every diagonal whose deletion leaves the merged piece convex. Deleting a diagonal can only change the interior angle at its two endpoints, so one pass over the diagonals suffices, and the result is guaranteed within a factor of four of the fewest convex pieces possible — whatever order the diagonals are visited in, which is why no search is worth doing. A constrained edge is never deleted, so a partition can be shaped by the constraints the triangulation was built with. `Polygon::convexPartition()` and `PolygonWithHoles::convexPartition()` are shorthands for `triangulation().convexPartition()`; on a region, the pieces cover only what has area, so the holes are where there is no piece and a slit appears in none of them. A convex polygon comes back as a single piece. On a random simple polygon this halves the piece count against the triangulation, which is what the non-convex Minkowski sum is charged for.
