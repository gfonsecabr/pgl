...
- `voronoiEdges()` returns the segments and rays that diagram is made of, in no particular order and with nothing said about how they meet. The Delaunay dual itself is cheap; overlaying the edges into an arrangement is what costs, so a caller that only wants to draw the diagram, or to feed the edges to something of its own, should stop here.
