...
- `insert(p)` adds point `p` as a new vertex, subdividing the triangle whose interior contains it (one triangle becomes three) or the edge containing it (each incident triangle splits in two, and the edge's two halves inherit its constrained flag and label). A point strictly outside the convex hull grows the hull, joining `p` to every hull edge it sees; for a polygon triangulation that growth lies outside the domain and stays hidden from the public view, like the hull-fill triangles. It returns `false` — leaving the triangulation unchanged — if `p` is already a vertex or lies strictly inside the carved-away exterior of a polygon. `insertDelaunay(p)` additionally restores the constrained Delaunay property around the new vertex by Lawson flips (never flipping constrained edges): a triangulation that was constrained Delaunay stays constrained Delaunay.
