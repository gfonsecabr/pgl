...
- Cells: `vertexCount()`, `halfedgeCount()`, `edgeCount()` (half the halfedge count) and `faceCount()` (the unbounded face included), plus the ranges `vertices()`, `halfedges()` and `faces()` of every handle in index order. Indexing with a handle gives the cell's geometry: `a[v]` is the position of a vertex, and `a[h]` is the edge of a halfedge as a `Segment` carrying the edge label. Twin halfedges give the same segment, since a segment is unoriented.
