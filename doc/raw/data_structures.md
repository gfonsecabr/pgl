...
- `locate(p)` returns a triangle containing point `p`, or none if `p` is outside, via a randomized visibility walk. The walk starts where the previous query ended, which is fast for queries that follow one another. `buildPointLocation()` replaces the walk by a descent of a Kirkpatrick hierarchy of triangulations, built in linear expected time and space and answering in logarithmic time. `hasPointLocation()` reports whether it is in place, `hasCurrentPointLocation()` whether it was drawn against the mesh as it now stands, and `clearPointLocation()` releases it. The structure is not updated when the triangulation is modified: it stays usable, the walk resuming from where the descent lands, but that walk lengthens as the mesh grows away from it.
...
- `voronoiDiagram()` returns the unbounded Arrangement dual to the current triangulation. The triangulation must be nonempty and its current real triangles must form a Delaunay triangulation of all stored vertices (otherwise you get the circumcentric dual). The result uses exact rational vertices by default for integral input. Each face is labeled with the point that generated its Voronoi cell, so `diagram.label(diagram.locateFace(q))` returns the nearest site. At a Voronoi edge or vertex, `locateFace` selects one tied site by its usual infinitesimal-perturbation rule; use `locateCell`, then inspect the incident faces, to recover every tied site. The free function of the same name in [algorithms](algorithms.md) computes this diagram straight from a container of points, and `powerDiagram` beside it covers weighted sites.

- `voronoiEdges()` returns the segments and rays that diagram is made of, in no particular order. They meet only at shared endpoints, so a caller that only wants to draw the diagram, or to feed the edges to something of its own, can stop here and skip building the arrangement around them.
