...
also the index of the shape the node owns. A tree therefore holds at most
`2^32 - 1` shapes.
...
- `locate(p)` returns a triangle containing point `p`, or none if `p` is outside, via a randomized visibility walk, in $O(V)$ time for a triangulation of $V$ vertices. The walk starts where the previous query ended, which is fast for queries that follow one another. `buildPointLocation()` replaces the walk by a descent of a Kirkpatrick hierarchy of triangulations, built in $O(V)$ time and space. A query then takes $O(\log V)$ time when its coordinates are exact and of the triangulation's own point type, the mesh has not been modified since, and `p` lies strictly inside the convex hull of the vertices or outside the box the hierarchy encloses them in; any other query takes $O(V)$ time. `hasPointLocation()` reports whether it is in place, `hasCurrentPointLocation()` whether it was drawn against the mesh as it now stands, and `clearPointLocation()` releases it. The structure is not updated when the triangulation is modified: it stays usable, the walk resuming from where the descent lands, but that walk lengthens as the mesh grows away from it.
