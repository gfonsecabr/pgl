...
- `squaredHausdorffDistance<ResultNumber>(Shape)`{Polygon}: Returns the squared Hausdorff distance. Defined for every pair among `Point`, `Segment`, `OrientedSegment`, `Rectangle`, `Triangle`, `Convex`, and `HalfplaneIntersection` — all bounded and convex. Neither operand may be empty: an empty `Rectangle`, `Convex` or `HalfplaneIntersection` is a precondition violation. A `HalfplaneIntersection` operand must also be bounded, and an unbounded one throws `std::logic_error`. Not defined for `Line`, `OrientedLine`, `Ray`, or `Halfplane`, whose Hausdorff distance is infinite, nor yet for `Disk`, `MonotoneChain`, `Polyline`, `Polygon`, `PolygonWithHoles`, or `PolygonSet`.
