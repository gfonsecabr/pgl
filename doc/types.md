<!-- AUTO-GENERATED from doc/raw/types.md by doc/raw/doxylink.py — do not edit; edit the raw version and regenerate. -->
...
When that doubly promoted type is an arbitrary precision one — which is the case for `int64_t`, `pgl::int128` and [`pgl::BigInt`](https://gfonsecabr.github.io/pgl/classpgl_1_1BigInt.html "Arbitrary precision signed integer.") coordinates, and for any `pgl::Rational<pgl::BigInt>` — the inCircle *sign* is first attempted in floating point, carrying an error bound alongside the value. The sign is reported from that attempt only when the bound proves it, and the exact determinant is computed otherwise, so the answer is the same either way; near-degenerate and exactly cocircular inputs pay for the attempt on top of what they always cost. That trade is a good one by a wide margin: a Delaunay triangulation over `pgl::ERational` coordinates builds twice as fast, while an input the filter can never decide — four exactly cocircular points — makes the predicate some 20% dearer. Coordinates whose determinant already fits in a machine integer skip the attempt entirely, their exact arithmetic being cheaper than the filter.

The same filter guards the degree-two sign predicates — orientation, cross product and dot product, hence [`collinear`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a93433f09e21f851c07f464d61e8f938f "Tests whether three points are collinear."), [`sameDirection`](https://gfonsecabr.github.io/pgl/namespacepgl.html#a4e68e2d3cc707f5eec3330cef58e9482 "Tests whether the directions a1 -> a2 and b1 -> b2 are parallel.") and every side test built on them. Those promote only once, so it engages for `pgl::int128` and [`pgl::BigInt`](https://gfonsecabr.github.io/pgl/classpgl_1_1BigInt.html "Arbitrary precision signed integer.") coordinates and for `pgl::Rational<pgl::BigInt>`, and stays out of the way of everything narrower. The gain is again largest where the exact fallback is dearest: over `pgl::ERational` coordinates, locating a point in a triangulation is more than twice as fast, the range queries around 1.7 times, and a constrained Delaunay triangulation builds about 20% faster. Exactly degenerate inputs — three collinear points, two parallel or perpendicular vectors — are the ones the filter can never decide, and they pay for the attempt on top of the exact evaluation.
