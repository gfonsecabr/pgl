...
`s.with<Number>()` returns a copy of `s` with coordinates of type `Number`, keeping the point and shape labels. `s.withLabel<Label>()` changes the shape label type and `s.withPointLabel<Label>()` the point label type. The label of a point, a [`Shape`](https://gfonsecabr.github.io/pgl/structpgl_1_1Shape.html "Runtime variant wrapper over the supported primitive shapes.") or an [`EmptyShape`](https://gfonsecabr.github.io/pgl/structpgl_1_1EmptyShape.html "The empty set of points in the plane.") is its point label, so for them the two methods are the same.

Labels convert as in a converting construction: they are copied into the new type, default-constructed when there was no label, and dropped by [`NoLabel`](https://gfonsecabr.github.io/pgl/structpgl_1_1NoLabel.html "Sentinel type used when a point carries no extra label."). A label type that cannot be built from the old one does not compile.
...
When the exact arithmetic a predicate would need is an arbitrary precision one — `pgl::int128` and [`pgl::BigInt`](https://gfonsecabr.github.io/pgl/classpgl_1_1BigInt.html "Arbitrary precision signed integer.") coordinates, any `pgl::Rational<pgl::BigInt>`, and the incircle test on `int64_t`, which promotes twice — the fundamental predicates (orientation test, incircle test...) first attempt a floating-point evaluation carrying an error bound, and compute the exact determinant only when that bound does not prove the sign. Coordinates whose determinant fits in a fixed-width type skip the attempt, their exact arithmetic being cheaper than the filter. Either way the answer is the same one.
...
None of them changes the value a [`Rational`](https://gfonsecabr.github.io/pgl/classpgl_1_1Rational.html "Exact rational number class template.") represents, so choosing among them is only ever a performance decision. All work in constant expressions.
