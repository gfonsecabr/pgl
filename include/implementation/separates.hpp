...
constexpr bool Rectangle<PointType, LabelType>::separates(const OtherRectangle& other) const {
    ...
    const bool splits_vertically =
        ...
        !(other.min().x() < min().x()) &&
        !(max().x() < other.max().x());
    return splits_vertically && !empty() && !other.empty();
}
...
constexpr bool HalfplaneIntersection<PointType, LabelType>::separates(const OtherRay& other) const {
    ...
    if (empty() || halfplanes_.empty() || contains(other.source())) {
        return false;
    }
...
constexpr bool HalfplaneIntersection<PointType, LabelType>::separates(const OtherHalfplane& other) const {
    ...
    if (empty() || halfplanes_.empty() || halfplanes_.size() == 1) {
...
constexpr bool Point<Number, Label>::separates(const OtherRegion& other) const {
    ...
    if (other.empty() || !other.isDegenerate()) {
        return false;
    }
...
bool Rectangle<PointType, LabelType>::separates(const OtherSet& other) const {
    // No empty short-circuit here: a set of regions may already be in several
    // pieces, and removing the empty set from one of those still leaves it
    // disconnected. The cell engine decides, reached through the empty polygon.
    return asPolygon().separates(other);
}
