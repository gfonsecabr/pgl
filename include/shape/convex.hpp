...
struct Convex {
    ...
    constexpr Convex(const Convex<OtherPointType, OtherLabelType>& other)
        : points_(detail::convertVertices<PointType>(other.vertices())), label_(detail::copyLabel<LabelType>(other)) {}

    /**
     * @brief Returns a copy with coordinates of type @p Number.
     *
     * The point and polygon labels are kept.
     *
     * @tparam Number Coordinate type of the result.
     */
    template <class Number>
    [[nodiscard]] constexpr Convex<detail::with_number_t<PointType, Number>, LabelType> with() const {
        return Convex<detail::with_number_t<PointType, Number>, LabelType>(*this);
    }

    /**
     * @brief Returns a copy whose polygon label has type @p Label.
     *
     * The label converts as in a converting construction: it is copied into
     * @p Label, default-constructed when this polygon has none, and dropped when
     * @p Label is @ref NoLabel.
     *
     * @tparam Label Polygon label type of the result.
     */
    template <class Label>
        requires(detail::can_copy_label_v<Label, LabelType>)
    [[nodiscard]] constexpr Convex<PointType, Label> withLabel() const {
        return Convex<PointType, Label>(*this);
    }

    /**
     * @brief Returns a copy whose points' labels have type @p Label.
     *
     * Each point label converts as in a converting construction; the polygon
     * label is kept.
     *
    ...
    [[nodiscard]] constexpr Convex<detail::with_point_label_t<PointType, Label>, LabelType> withPointLabel() const {
        return Convex<detail::with_point_label_t<PointType, Label>, LabelType>(*this);
    }
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::intersectionTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::regularizedIntersectionTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::regularizedUnionTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::differenceTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::symmetricDifferenceTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::squaredDistanceTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::distanceL1Tag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::distanceLInfTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::squaredHausdorffDistanceTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::hausdorffDistanceL1Tag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::hausdorffDistanceLInfTag, Convex, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::closestPointsTag, Convex, OtherShape>
    ...
    template <class ResultNumber = NumberType, ShapeConcept OtherShape>
        requires detail::Receives<detail::closestSegmentsTag, Convex, OtherShape>
    ...
    /**
     * @brief Returns the integer points the convex polygon contains.
     *
     ...
     *
     * Same counterclockwise vertex sequence as @ref vertices(), with no heap
     * allocation. The convex polygon is itself a vertex range, so this is just
     ...
     * A convex polygon erodes to the region its own constraints leave once
     * each is moved in.
     *
     ...
     * — so it answers with a set of regions like every other union. A convex
     ...
     *
     * A convex polygon is the highest-ranked of the three bounded convex
     * regions, so it owns its pairs with all three; the pairs with an operand
     * above it are defined there and reached through the forwarding overload
     * below. See @ref Polygon::symmetricDifference for the contract.
     *
     * @tparam ResultNumber The number type for the result.
