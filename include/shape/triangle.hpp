...
struct Triangle {
     ...
     *
     * The point and triangle labels are kept.
     *
     * @tparam Number Coordinate type of the result.
     */
    template <class Number>
    [[nodiscard]] constexpr Triangle<detail::with_number_t<PointType, Number>, LabelType> with() const {
        return Triangle<detail::with_number_t<PointType, Number>, LabelType>(*this);
    }

    /**
     * @brief Returns a copy whose triangle label has type @p Label.
     *
     * The label converts as in a converting construction: it is copied into
     * @p Label, default-constructed when this triangle has none, and dropped when
     * @p Label is @ref NoLabel.
     *
     * @tparam Label Triangle label type of the result.
     */
    template <class Label>
        requires(detail::can_copy_label_v<Label, LabelType>)
    [[nodiscard]] constexpr Triangle<PointType, Label> withLabel() const {
        return Triangle<PointType, Label>(*this);
    }

    /**
     * @brief Returns a copy whose points' labels have type @p Label.
     *
     * Each point label converts as in a converting construction; the triangle
     * label is kept.
     *
    ...
    [[nodiscard]] constexpr Triangle<detail::with_point_label_t<PointType, Label>, LabelType> withPointLabel() const {
        return Triangle<detail::with_point_label_t<PointType, Label>, LabelType>(*this);
    }
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::intersectionTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::regularizedIntersectionTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::regularizedUnionTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::differenceTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::symmetricDifferenceTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::squaredDistanceTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::distanceL1Tag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::distanceLInfTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::squaredHausdorffDistanceTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::hausdorffDistanceL1Tag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::hausdorffDistanceLInfTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::closestPointsTag, Triangle, OtherShape>
    ...
    template <class ResultNumber = NumberType, ShapeConcept OtherShape>
        requires detail::Receives<detail::closestSegmentsTag, Triangle, OtherShape>
    ...
    /** @brief Tests whether some defining point of this shape is the point @p other holds. */
    template <ShapeConcept OtherShape>
    [[nodiscard]] constexpr bool verticesContain(const OtherShape& other) const {
    ...
    /**
     * @brief Returns the integer points the triangle contains.
     *
    ...
    /** @brief Adds this triangle's three constraints to a half-plane intersection without deriving vertices. */
    ...
    template <class ApproximateNumber = double, detail::NonConvexPolygonalConcept OtherShape>
        requires requires(const OtherShape& o, const Triangle& self) {
     ...
     * that overlap in a corner make an L, and two that are apart make two pieces
     * — so it answers with a set of regions like every other union. A triangle
     * owns this pair and the one with a @ref Rectangle, being the higher-ranked
     * of the two; the pairs with an operand above it are defined there and
     * reached through the forwarding overload below. See @ref Polygon::regularizedUnion
     * for the contract.
     *
     * @tparam ResultNumber The number type for the result.
     ...
     * hand the pair to the way @ref regularizedUnion does: a triangle has to state it
     * against every region itself. It states it once, over all six of them, by
     * going through its polygon spelling — the same conversion every other
     * operation makes, and one that costs nothing, the three vertices already
     * being in canonical polygon order. See @ref Polygon::difference for the
     * contract.
     *
     * @tparam ResultNumber The number type for the result.
     ...
     *
     * A triangle owns this pair and the one with a @ref Rectangle, being the
     * higher-ranked of the two; the pairs with an operand above it are defined
     * there and reached through the forwarding overload below. See
