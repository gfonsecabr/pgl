...
struct PolygonWithHoles {
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::intersectionTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::regularizedIntersectionTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::regularizedUnionTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::differenceTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::symmetricDifferenceTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::squaredDistanceTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::distanceL1Tag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::distanceLInfTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::squaredHausdorffDistanceTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::hausdorffDistanceL1Tag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::hausdorffDistanceLInfTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = division_result_t<NumberType>, ShapeConcept OtherShape>
        requires detail::Receives<detail::closestPointsTag, PolygonWithHoles, OtherShape>
    ...
    template <class ResultNumber = NumberType, ShapeConcept OtherShape>
        requires detail::Receives<detail::closestSegmentsTag, PolygonWithHoles, OtherShape>
    ...
    /** @brief Tests whether this shape's interior contains the interior of the segment @p other holds. */
    template <ShapeConcept OtherShape>
    [[nodiscard]] constexpr bool interiorContainsInterior(const OtherShape& other) const {
    ...
    /**
     * @brief Returns the region with vertices removed, within a Hausdorff
     ...
     * coordinate type. Its Hausdorff distance to this region is at most the
     ...
     * No ring is removed. The result satisfies @ref isValid when this region
     * does, and its rings meet only where this region's rings met.
     *
    ...
    [[nodiscard]] PolygonWithHoles simplified(const Tolerance& squaredTolerance) const;

    /** @brief Replaces the region by @ref simplified with the same tolerance. */
    template <class Tolerance>
     ...
     * approximate any other region.
     *
     * A cell is an integer position, so the coordinates must be whole numbers
     * too. An integer region rasterizes as it stands; one over a Rational or a
     * floating-point type is checked vertex by vertex and throws unless every
     * coordinate happens to be whole -- rounding one would move the region.
     *
     ...
     * @ref Polygon::difference for the contract.
     */
     ...
     * `A` is, however far `B` reaches, so a `PolygonSet` can hold it. That makes
     * the difference the second operation to take an unbounded operand, after
     * @ref intersection, and the only one where the latitude is one-sided —
     * `B ∖ A` is unbounded and has no overload, on this or any receiver.
     *
     * `A` is bounded, so only the part of `B` near it can matter: `B` is first
     * clipped to a box strictly containing the bounding rectangle, which leaves
     * `A ∖ B` untouched and makes it a convex polygon. The clip's corners are
     * whole numbers in the operand's own coordinate type, so the arrangement
     * stays exact whatever @p ResultNumber is. A half-plane intersection with
     * empty interior removes nothing from a regularized result, and gives back
     * @ref regularized.
     *
     * @tparam ResultNumber The number type for the result.
     * @param other The region to remove; may be unbounded, up to the whole plane.
     * @return The pieces of the difference, in canonical order.
     */
     ...
     * as one: see @ref difference(const OtherIntersection&) const, which bounds
     * it against this region before any arrangement is built. Removing a
     * half-plane can leave several pieces and can open a hole out into the rest
     * of the plane, which is why the result is a set of regions rather than one.
     */
     ...
     * round between them. See @ref Polygon::regularizedUnion for the full contract.
     *
     * @tparam ResultNumber The number type for the result.
     ...
     * above a region, and it states its operations over every operand at once, so
     * this hands the pair back to it rather than restating it. A union is
     * symmetric, so the order costs nothing — and going through the set is what
     * puts every component into one arrangement instead of folding this region
     * over them one at a time. See @ref Polygon::regularizedUnion for the contract.
     */
    template <class ResultNumber = division_result_t<NumberType>, PolygonSetConcept OtherSet>
    ...
    template <class ResultNumber = division_result_t<NumberType>, PolygonWithHolesConcept OtherRegion>
    [[nodiscard]] PolygonSet<Point<ResultNumber, typename PointType::LabelType>>
    regularizedIntersection(const OtherRegion& other) const;
     ...
     *
     * This is the region-valued `regularizedIntersection`, so it answers
     * `intersection.regularizedIntersection(region)` too — see
     * @ref regularizedIntersection(const OtherPolygon&) const. The clip's corners are
     * whole numbers in the operand's own coordinate type, so the arrangement
     ...
     *
     * A half-plane is the one-constraint half-plane intersection, and is handled
     * as one: see @ref regularizedIntersection(const OtherIntersection&) const, which
     ...
     *
     * This is the region-valued `regularizedIntersection`, so it answers
     * `halfplane.regularizedIntersection(region)` too — the half-plane's own
     ...
     * and returned as a region with holes. See
     ...
     *
     * The precondition a polygon receiver states as *nondegenerate* is a little
     * more than that here, and for the same reason: this region must be a
     * **body**, the closure of a connected non-empty interior. A region with no
     * area is not one, and neither is one whose slits cut its interior in two —
     * `[0,8]² ∖ (0,4)²` written with a hole sharing two of the outer ring's
     * edges is the plainest example. Either way the sum can fall into several
     * pieces and one of them is what comes back. A nondegenerate **operand**
     * satisfies the precondition on its own, whatever this region is: one body
     * on either side is all the single-region argument needs.
     *
     * @tparam ResultNumber The number type for the result.
     * @param other The shape to sum with.
     * @return The sum, as one region.
     * @pre This region or the operand is a body.
     */
    template <class ResultNumber = division_result_t<NumberType>, PolygonConcept OtherPolygon>
     ...
     * A region eroded by a bounded operand needs a @ref PolygonSet, since an
