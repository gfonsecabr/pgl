...
static const Rectangle box(Point(0, 0), Point(4, 4));
...
TEST_CASE("Polyline and Rectangle containment and intersection") {
    ...
    CHECK(Rectangle(Point(-1, -1), Point(5, 5)).interiorContains(zig));
    ...
    const Rectangle below(Point(0, -4), Point(4, 0));
    CHECK(zig.intersects(below));
    CHECK_FALSE(zig.interiorsIntersect(below));

    const Rectangle away(Point(6, 0), Point(8, 2));
...
TEST_CASE("Rectangle separates a polyline") {
    ...
    const Rectangle apexBox(Point(1, 1), Point(3, 3));
    ...
    SUBCASE("set semantics: the loop reconnects around a clipped corner") {
        ...
        const Rectangle cornerBox(Point(-1, -1), Point(1, 1));
        CHECK_FALSE(cornerBox.separates(loop));       // one bite, arc survives
        const Rectangle sideBox(Point(1, -1), Point(3, 1));
        CHECK(sideBox.separates(loop) == false);      // bottom bite only
        const Rectangle throughBox(Point(1, -1), Point(3, 5));
...
TEST_CASE("Polyline and Rectangle distance") {
    ...
    const Rectangle diagonal(Point(6, 4), Point(8, 6));
