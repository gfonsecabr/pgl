...
TEST_CASE("Point and Polygon containment predicates") {
    using Point = pgl::Point<int>;
    using Polygon = pgl::Polygon<Point>;

    const Polygon sq(std::vector<Point>{{0, 0}, {4, 0}, {4, 4}, {0, 4}});
    ...
    // A point never contains a (2D) polygon.
    CHECK_FALSE_MESSAGE(interior.contains(sq), interior, " contains ", sq);
}

TEST_CASE("Point and Polygon intersection predicates, both directions") {
    using Point = pgl::Point<int>;
    using Polygon = pgl::Polygon<Point>;
...
TEST_CASE("Point and Polygon never separate and never cross") {
    using Point = pgl::Point<int>;
    using Polygon = pgl::Polygon<Point>;
...
TEST_CASE("Point and Polygon intersection construction, both directions") {
    using Point = pgl::Point<int>;
    using Polygon = pgl::Polygon<Point>;
